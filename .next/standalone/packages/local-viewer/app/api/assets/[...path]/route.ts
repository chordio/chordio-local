...
/**
 * Static asset serving route for clone/prototype assets (fonts, images).
 *
 * URL pattern: /api/assets/{product}/{clones|prototypes}/{name}/fonts/{file}
 * Maps to:     products/{product}/{clones|prototypes}/{name}/assets/fonts/{file}
 *
...
export async function GET(
  ...
  // Minimum: product/type/name/subdir/file (e.g., chewy/clones/homepage/fonts/gordita-400.woff2)
  ...
  // Build the file path: products/{product}/{type}/{name}/assets/{rest...}
  const productsDir = resolveProductsDir();
  const filePath = path.join(productsDir, product, type, name, 'assets', ...rest);
